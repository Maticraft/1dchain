...
# Model name
model_name = 'GAN_from_pos_autoencoder_fft_tf'
...
generator_optimizer = torch.optim.Adam(generator.parameters(), lr=params['lr'])
decoder_optimizer = torch.optim.Adam(discriminator.parameters(), lr=params['lr'])
...
for epoch in range(1, params['epochs'] + 1):
    ...
    plot_test_eigvals(SpinLadder, encoder, generator, x_axis, x_values, DEFAULT_PARAMS, eigvals_path, device=device, xnorm=xnorm, ylim=ylim)
    ...
    plot_test_matrices(SpinLadder(**DEFAULT_PARAMS).get_hamiltonian(), encoder, generator, save_path_rec=ham_auto_path, save_path_diff=ham_diff_path, device=device)
