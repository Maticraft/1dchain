...
for sample_idx in range(num_samples):    
    with torch.no_grad():
        ...
        subsamples_dir = os.path.join(sample_dir, 'subsamples')
        current_conductance = h_noisy_conductance.clone()
        for k in range(n_iterations):
            noise_k = (1. - (k / n_iterations))*real_noise_amplitude 
            params_map = model_c2h(current_conductance, noise_k, None, matrix_output=False)
            ...
            current_conductance = cmap_normalize(predicted_cmap).unsqueeze(0)

            if k % 10 == 0:
                for i, cmap_config in enumerate(conductance_config['cmap_list']):
                    predicted_conductance_path = os.path.join(subsamples_dir, f'improved_conductance_step_{k}')
                    ...
                    plot_conductance_map(
                        ...
                        xlabel="$B$ [mV]",
                        ylabel="$E_F$ [meV]",
                        title=f'Noise amplitude: {noise_k.item():.2f}'
                    )
