...
from src.models.denoiser import train_denoising_model, test_denoising_model
...
save_dir = './denoiser/quantum_dots/3dots1level_majoranas_gap_pol_verified'
loss_file = 'loss.txt'
...
# Model name
model_name = 'QDH-1lvl-no-interlevel_DiT_28_ham_flat_param_mock_noise_strength_percentage0_2_lr1e-4decreasing'
...
for epoch in range(0, params['epochs'] + 1):
    train_loss = train_denoising_model(
        model,
        noise_generator,
        train_loader,
        optimizer,
        device,
        epoch,
        noise_max_strength=params['max_noise_amplitude'],
        mock_model_noise_strength_input=params['mock_model_noise_strength_input'],
    )
    test_loss, test_ref_loss = test_denoising_model(
        model,
        noise_generator,
        test_loader,
        device,
    ...
    if epoch % 10 == 0:
        ...
        noise = noise_generator.generate_noise(1, h_torch_normalized.shape[-1], device=device)
        ...
        for dir_path in [amplitude_noise_dir, full_noise_dir]:
            ...
            if 'full_noise' in dir_path:
                noise_amplitude = torch.tensor([[1.]], device=device)
            else:
                noise_amplitude = torch.tensor([[params['max_noise_amplitude']]], device=device)

            h_torch_noisy = (1-noise_amplitude)*h_torch_normalized + noise_amplitude * noise

            if params['mock_model_noise_strength_input']:
                noise_amplitude = torch.zeros_like(noise_amplitude)
