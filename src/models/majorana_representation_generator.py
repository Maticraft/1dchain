    ...
    def __init__(
        ...
        **kwargs: t.Any
        ...
        self.inter_site_converters = nn.ModuleList([
            nn.Conv1d(self.hidden_channels, self.num_inter_site_params, kernel_size=2, stride=1, dilation=interaction_range)
...
class MajoranaRepresentationHamiltonianConstructor(nn.Module):
    ...
    def generate_random_hamiltonian(self, num_samples: int, seq_size: int, device: t.Optional[torch.device] = None) -> torch.Tensor:
        '''
        Args:
          num_samples: int
          seq_size: int
